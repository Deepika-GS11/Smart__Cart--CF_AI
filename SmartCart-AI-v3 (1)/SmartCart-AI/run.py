...
def check_dependencies():
    ...
    required = ["flask", "flask_login", "flask_sqlalchemy", "flask_cors",
                "pandas", "numpy", "sklearn", "scipy", "matplotlib", "faker"]
    missing = []
    for pkg in required:
        try:
            __import__(pkg)
...
def check_datasets():
    ...
    if not all_ok:
        ...
        subprocess.check_call(
            [sys.executable, os.path.join(BASE_DIR, "utils", "generate_dataset.py")])
...
def run_preprocessing():
    ...
    if os.path.exists(preprocessed):
        ...
        subprocess.check_call(
            [sys.executable, os.path.join(BASE_DIR, "utils", "preprocess.py")])
